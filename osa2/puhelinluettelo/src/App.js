...
const App = () => {
  ...
  const addPerson = (event) => {
    ...
    if ((personAlreadyExists !== undefined)) {
      let replaceNumber = window.confirm(`${newName} is already added to phonebook,
      ...
      if (replaceNumber) {
        ...
        personService
          ...
          .then(returnedPerson => {
            ...
            setNewName("");
            setNewNumber("");
            setNotification(`Persons ${changedPerson.name} number changed to ${changedPerson.number}`);
  ...
  const deletePerson = (id) => {
    ...
    personService
      ...
      .catch(error => {
        setNotification(`Person ${personToDelete.name} was already removed from server`);
        setNotificationType("error")
        setTimeout(() => {
          setNotification(null);
        }, 5000);
  ...
  return (
    <div>
      ...
      />
      <h3>Numbers</h3>
      <Persons persons={(newFilter.length === 0) ? persons.concat() : filterPersons()}
